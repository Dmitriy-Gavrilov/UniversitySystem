...
@router.get("/full_info/{teacher_id}",
            summary="Получить данные преподавателя вместе с предметами",
            response_model=ResponseTeacherSchema)
async def get_full_info_teacher_by_id(teacher_id: int, session: AsyncSession = Depends(get_session)):
    teacher_service = TeacherService(Repository[Teacher](Teacher, session))
    return await teacher_service.get_full_info(teacher_id, Repository[Assignment](Assignment, session),
